  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* learn more */}
        {!isExpanded && (
          <div className="absolute inset-0 flex items-start justify-center bg-background bg-opacity-60 p-[1vw] opacity-0 transition-opacity duration-300 group-hover:opacity-100">
            <span className="text-base font-semibold text-foreground underline sm:text-lg md:text-xl">Learn More</span>
          </div>
        )}
        ...
        {isExpanded && (
          ...
          >
            ...
            <div className="mt-[1.5vw] cursor-pointer text-left" onClick={(e) => e.stopPropagation()}>
              ...
              <p className="cursor-pointer text-xs text-foreground sm:text-sm md:text-base" onClick={() => setExpanded(false)}>
                {member.major}
              </p>
              <a
              ...
              <p
                className="mt-[1vw] cursor-pointer text-xs text-foreground sm:text-xs md:text-xs"
                onClick={() => setExpanded(false)}
                style={{ marginTop: "1vw" }}
              >
                {member.description}
